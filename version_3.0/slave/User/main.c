...
static uint8_t build_response(const AppHeader *r, const uint8_t *p, uint8_t *out)
{
    ...
    if (r->type == APP_MSG_COMMAND && r->payload_len == sizeof(AppCommand)) {
        ...
        {
            uint8_t mask = (uint8_t)(c.value & 0x0Fu), next, i;

            if (!app_relay_apply(relay_state, c.value, &next)) {
                a.result = APP_RESULT_INVALID_PARAM;
            } else {
                for (i = 1u; i <= 4u; i++) {
                    if (mask & (1u << (i - 1u))) {
                        relay_set(i, (next & (1u << (i - 1u))) ? 1u : 0u);
                    }
                }
                a.value = relay_state;
        ...
        case APP_CMD_GET_RELAY_STATE:
            a.value = relay_state;
            dbg_s("RELAY GET state=");
            dbg_u32(relay_state);
            dbg_s("\r\n");
            break;
