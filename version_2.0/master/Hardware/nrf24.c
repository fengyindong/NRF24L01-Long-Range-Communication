...
#define CMD_NOP           0xFFu   /* 空操作，用于读取状态 */

/* ------------------------- 寄存器地址定义 ------------------------- */
#define REG_CONFIG        0x00u   /* 配置寄存器 */
#define REG_EN_AA         0x01u   /* 自动应答使能寄存器 */
#define REG_EN_RXADDR     0x02u   /* 接收地址使能寄存器 */
#define REG_SETUP_AW      0x03u   /* 地址宽度设置寄存器 */
#define REG_SETUP_RETR    0x04u   /* 自动重发设置寄存器 */
#define REG_RF_CH         0x05u   /* 射频通道寄存器 */
#define REG_RF_SETUP      0x06u   /* 射频设置寄存器 */
...
#define REG_DYNPD         0x1Cu   /* 动态载荷使能寄存器 */
...
#define STATUS_RX_DR      0x40u   /* 接收数据就绪中断位 */
#define STATUS_TX_DS      0x20u   /* 发送数据完成中断位 */
#define STATUS_MAX_RT     0x10u   /* 达到最大重发次数中断位 */
...
#define NRF_RETR_VALUE        0x35u  /* 先保持已经验证可通信的自动重发参数 */

/* CE拉低：停止接收或结束发送脉冲。 */
static void ce_low(void) { GPIO_ResetBits(NRF_CE_PORT, NRF_CE_PIN); }
/* CE拉高：在PRX模式进入持续接收，在PTX模式启动发送。 */
static void ce_high(void) { GPIO_SetBits(NRF_CE_PORT, NRF_CE_PIN); }
/* CSN拉低：开始一笔SPI命令事务。 */
static void csn_low(void) { GPIO_ResetBits(NRF_CSN_PORT, NRF_CSN_PIN); }
/* CSN拉高：结束当前SPI事务并让无线芯片执行命令。 */
static void csn_high(void) { GPIO_SetBits(NRF_CSN_PORT, NRF_CSN_PIN); }
...
uint8_t nrf24_is_present(void)
{
    ...
    if ((status == 0x00u || status == 0xFFu || (status & 0x80u)) &&
        (status2 == 0x00u || status2 == 0xFFu || (status2 & 0x80u))) return 0;
